...
var chatRole = "{role}";
var currentDifficulty = "{difficulty}"; // Loaded from DB
...
function addEvaluationCard(evalData) {{
  ...
  card.innerHTML = `
    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
      ...
      <div style="font-size:0.75rem; font-weight:600; color:#94a3b8; background:rgba(255,255,255,0.05); padding:2px 8px; border-radius:4px; border:1px solid rgba(255,255,255,0.1);">
        DIFF: <span id="difficultyBadge" style="color:#3b82f6;">${{currentDifficulty.toUpperCase()}}</span>
      </div>
    </div>
...
async function sendChatAnswer() {{
  ...
  try {{
    ...
    if (evalData && evalData.technical_score !== undefined) {{
      ...
      // Step 3 Logic: Adjust difficulty
      var avgScore = (evalData.technical_score + evalData.clarity_score + evalData.communication_score) / 3;
...
function showCodingEnvironment(problemText) {{
    var codingArea = document.getElementById('codingArea');
    if (!codingArea) {{
        codingArea = document.createElement('div');
        codingArea.id = 'codingArea';
        codingArea.style.cssText = 'margin-top:20px; border-top:1px solid rgba(124,58,237,0.3); padding-top:20px;';
        codingArea.innerHTML = `
            <div style="font-family:'Outfit'; font-size:1.1rem; margin-bottom:12px; color:#10b981; display:flex; align-items:center; gap:8px;">
              <span>💻</span> AI Coding Workspace
            </div>
            <div id="monacoEditor" style="height:350px; border-radius:12px; border:1px solid rgba(124,58,237,0.2); overflow:hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.2);"></div>
            <div style="margin-top:12px; display:flex; gap:10px;">
                <button id="runCodeBtn" style="background:#10b981; color:white; border:none; padding:10px 20px; border-radius:8px; font-weight:600; cursor:pointer; flex:1; transition:0.2s;">▶ Run Code</button>
                <button id="closeCodingBtn" style="background:rgba(239,68,68,0.1); color:#f87171; border:1px solid rgba(239,68,68,0.2); width:44px; border-radius:8px; cursor:pointer;">✕</button>
            </div>
            <div id="codingResult" style="margin-top:12px; display:none;"></div>
        `;
        document.querySelector('.chat-messages').parentNode.appendChild(codingArea);
        
        // Load Monaco via script tag first if not present
...
function initMonaco() {{
    ...
    document.getElementById('runCodeBtn').onclick = submitCode;
    document.getElementById('closeCodingBtn').onclick = () => {{ document.getElementById('codingArea').style.display = 'none'; }};
}}
...
async function submitCode() {{
    ...
    resultDiv.innerHTML = '<div style="color:#94a3b8; font-size:0.9rem;">Analysing code...</div>';
    
    try {{
        var resp = await fetch(window.location.origin + '/api/interview/run_code', {{
            method: 'POST',
            headers: {{ 'Content-Type': 'application/json' }},
            body: JSON.stringify({{ problem: currentProblem, code: code }})
        }});
        var data = await resp.json();
        
        var color = data.correctness === 'Pass' ? '#10b981' : (data.correctness === 'Fail' ? '#ef4444' : '#f59e0b');
        resultDiv.innerHTML = `
            <div style="background:rgba(255,255,255,0.03); border:1px solid rgba(124,58,237,0.2); border-radius:12px; padding:16px;">
                <div style="font-weight:700; color:${{color}}; margin-bottom:8px; font-size:1.1rem;">Coding Evaluation: ${{data.correctness}}</div>
                <div style="display:flex; gap:12px; margin-bottom:12px;">
                    <div style="background:rgba(0,0,0,0.2); padding:6px 12px; border-radius:6px; font-size:0.8rem;">Complexity: <strong style="color:#3b82f6;">${{data.runtime_complexity}}</strong></div>
                    <div style="background:rgba(0,0,0,0.2); padding:6px 12px; border-radius:6px; font-size:0.8rem;">Quality: <strong style="color:#8b5cf6;">${{data.code_quality}}/10</strong></div>
                </div>
                <div style="font-size:0.9rem; padding:12px; background:rgba(0,0,0,0.2); border-left:3px solid #3b82f6; color:#e2e8f0; border-radius:4px;">
                    <strong>Suggestion:</strong> ${{data.suggestion}}
                </div>
            </div>
        `;
    }} catch (e) {{
        resultDiv.innerHTML = '<div style="color:#ef4444;">Evaluation failed. Please check your connection.</div>';
    }} finally {{
        btn.disabled = false;
