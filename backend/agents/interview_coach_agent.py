...
<div class="container">
  ...
  <!-- ═══ AI INTERVIEW CHAT ═══ -->
  <!-- Portfolio meta for personalized questions -->
  <div id="portfolioMeta" data-projects="[]" style="display:none;"></div>
  <div class="ai-chat-section" id="aiChatSection">
    <div class="ai-chat-header" style="flex-wrap:wrap;gap:8px;">
      <span style="font-size:1.4rem">🤖</span>
      <h3>AI Interview Coach</h3>
      <span class="live-dot"></span>
      <span class="live-label">Live</span>
      <div style="margin-left:auto;display:flex;gap:8px;">
        <button id="endSessionBtn" style="background:rgba(124,58,237,0.15);color:#a78bfa;border:1px solid rgba(124,58,237,0.3);padding:6px 14px;border-radius:8px;font-size:0.8rem;font-weight:600;cursor:pointer;">📊 Final Report</button>
        <button id="transcriptBtn" style="background:rgba(6,182,212,0.1);color:#22d3ee;border:1px solid rgba(6,182,212,0.3);padding:6px 14px;border-radius:8px;font-size:0.8rem;font-weight:600;cursor:pointer;">📋 Transcript</button>
      </div>
    </div>
    <div class="chat-messages" id="chatMessages">
      <div class="chat-empty" id="chatEmpty">
        Click <strong>"Start AI Interview"</strong> below to begin a personalized real-time interview with an adaptive AI interviewer.<br/>
        <span style="font-size:0.8rem;color:#64748b;margin-top:6px;display:block;">After each answer you'll receive a 5-metric evaluation with strengths &amp; improvements.</span>
      </div>
    </div>
    <div class="chat-input-area">
      <textarea id="chatInput" placeholder="Type your answer here... (Shift+Enter for new line, Enter to submit)" rows="2" disabled></textarea>
      <button class="chat-send-btn" id="chatSendBtn" onclick="sendChatAnswer()" disabled>Submit Answer</button>
    </div>
    <div style="padding: 12px 24px; text-align:center; display:flex; gap:12px; justify-content:center; flex-wrap:wrap;">
...
<!-- Final Report Modal -->
<div id="reportModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.8);z-index:9999;align-items:center;justify-content:center;padding:20px;">
  <div style="background:#12121a;border:1px solid rgba(124,58,237,0.3);border-radius:20px;width:100%;max-width:600px;max-height:90vh;overflow-y:auto;padding:28px;position:relative;">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">
      <div style="font-family:'Outfit';font-size:1.4rem;font-weight:800;background:linear-gradient(to right,#a78bfa,#22d3ee);-webkit-background-clip:text;-webkit-text-fill-color:transparent;">🏆 Interview Report</div>
      <button onclick="document.getElementById('reportModal').style.display='none'" style="background:rgba(239,68,68,0.1);color:#f87171;border:1px solid rgba(239,68,68,0.2);padding:6px 12px;border-radius:8px;cursor:pointer;font-size:0.85rem;">✕ Close</button>
    </div>
...
<!-- Transcript Modal -->
<div id="transcriptModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.8);z-index:9999;align-items:center;justify-content:center;padding:20px;">
  <div style="background:#12121a;border:1px solid rgba(6,182,212,0.3);border-radius:20px;width:100%;max-width:700px;max-height:90vh;overflow-y:auto;padding:28px;">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">
      <div style="font-family:'Outfit';font-size:1.3rem;font-weight:800;color:#22d3ee;">📋 Session Transcript</div>
      <button onclick="document.getElementById('transcriptModal').style.display='none'" style="background:rgba(239,68,68,0.1);color:#f87171;border:1px solid rgba(239,68,68,0.2);padding:6px 12px;border-radius:8px;cursor:pointer;">✕ Close</button>
    </div>
...
def run_interview_coach_agent() -> list[dict]:
    ...
    for job in jobs:  # Process all internships as requested
        ...
        try:
            # Load existing session to get difficulty persistence
            sessions_data = read_yaml_from_github(INTERVIEW_INDEX_FILE)
            existing_sessions = sessions_data.get("interview_sessions", []) if isinstance(sessions_data, dict) else []
            current_difficulty = "medium"
            for sess in existing_sessions:
                if sess.get("company") == company and sess.get("role") == role:
                    current_difficulty = sess.get("current_difficulty", "medium")
                    break
            ...
            # Save index to GitHub after each job for resilience
            try:
                write_yaml_to_github(INTERVIEW_INDEX_FILE, {"interview_sessions": index})
            except Exception as exc:
                logger.error("InterviewCoachAgent: Failed to save index update — %s", exc)
